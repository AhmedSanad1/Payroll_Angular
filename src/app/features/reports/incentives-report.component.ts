...
import { IncentiveDeductionReportRow } from '../../core/models/report.models';
...
export class IncentivesReportComponent implements OnInit, OnDestroy {
  ...
  readonly rows = signal<IncentiveDeductionReportRow[]>([]);
  readonly loading = signal(false);
  readonly exportingPdf = signal(false);

  readonly theme = payrollGridTheme;
  readonly defaultColDef: ColDef = { sortable: true, resizable: true, flex: 1, minWidth: 120 };
  readonly colDefs = signal<ColDef<IncentiveDeductionReportRow>[]>([]);

  private gridApi?: GridApi<IncentiveDeductionReportRow>;
  ...
  onGridReady(event: GridReadyEvent<IncentiveDeductionReportRow>): void {
  ...
  load(): void {
    ...
    this.reportService.getIncentivesDeductionsReport(runId, this.departmentId()).subscribe((res) => {
  ...
  exportPdf(): void {
    ...
    this.reportService.getIncentivesDeductionsReportPdf(runId, this.departmentId()).subscribe({
      next: (pdf) => {
        this.exportingPdf.set(false);
        openPdf(pdf, `IncentivesDeductionsReport_${runId}.pdf`);
      },
      error: () => this.exportingPdf.set(false)
    });
  }

  private buildColDefs(): ColDef<IncentiveDeductionReportRow>[] {
    ...
    return [
      ...
      { field: 'deptIncentiveAmount', headerName: t('reports.deptIncentive'), valueFormatter: (p) => money(p.value) },
      { field: 'serviceIncentiveAmount', headerName: t('reports.serviceIncentive'), valueFormatter: (p) => money(p.value) },
      { field: 'attendanceAmount', headerName: t('reports.attendance'), valueFormatter: (p) => money(p.value) },
